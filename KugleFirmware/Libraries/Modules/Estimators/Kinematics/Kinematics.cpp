...
void Kinematics::ConvertBallToCoRvelocity(const float vel_ball[2], const float q[4], const float dq[4], float vel_CoR[2])
{
	_BallToCoRvelocity(vel_ball, q, dq, _params.model.CoR, vel_CoR);
}

// Supports overwriting, hence it is OK for the output array, 'vel_ball', to point to the array as the input array, 'vel_2L'
void Kinematics::ConvertCoRtoBallVelocity(const float vel_CoR[2], const float q[4], const float dq[4], float vel_ball[2])
{
	_ConvertCoRtoBallVelocity(vel_CoR, q, dq, _params.model.CoR, vel_ball);
}
