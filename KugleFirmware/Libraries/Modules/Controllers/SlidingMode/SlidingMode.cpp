...
SlidingMode::SlidingMode(Parameters& params, Timer * microsTimer) : _params(params), _microsTimer(microsTimer)
{
	Reset();
}
...
void SlidingMode::Step(const float q[4], const float dq_in[4], const float xy[2], const float dxy[2], const float q_ref_in[4], const float omega_ref_body[3], const float Jk, const float Mk, const float rk, const float Mb, const float Jbx, const float Jby, const float Jbz, const float Jw, const float rw, const float Bvk, const float Bvm, const float Bvb, const float l, const float g_const, const float COM_X, const float COM_Y, const float COM_Z, const float K[3], const float eta[3], const float epsilon[3], const bool continuousSwitching, const bool IncludeEquivalentControl, const float equivalentControlPct, const bool DisableQdotInEquivalentControl, const bool DisableOmegaXYInEquivalentControl, const lspc::ParameterTypes::slidingManifoldType_t manifoldType, float tau[3], float S[3])
{
    ...
    if (!(manifoldType == lspc::ParameterTypes::Q_DOT_INERTIAL_MANIFOLD || manifoldType == lspc::ParameterTypes::OMEGA_INERTIAL_MANIFOLD || manifoldType == lspc::ParameterTypes::Q_DOT_BODY_MANIFOLD || manifoldType == lspc::ParameterTypes::OMEGA_BODY_MANIFOLD)) {
    	tau[0] = 0;
    	tau[1] = 0;
    	tau[2] = 0;
    	S[0] = 0;
    ...
    if (manifoldType == lspc::ParameterTypes::Q_DOT_BODY_MANIFOLD) {
		...
		arm_mat_mult_f32(&devecPhiDQref_T_, &q_, &tmp3_); // devec*Phi(dq_ref)'*q
		arm_mult_f32((float*)K, tmp3, tmp3, 3); // K*devec*Phi(dq_ref)'*q
		arm_scale_f32(tmp3, 2.f, tmp3, 3); // Scale K with 2 to make gains tuned for the q_dot-based manifold comparable when changing to the omega-based manifold
		arm_add_f32(tmp3, sum3, sum3, 3); // sum += K*devec*Phi(dq_ref)'*q
    ...
    if (IncludeEquivalentControl) {
    	...
    	arm_scale_f32(tau_eq, equivalentControlPct, tau_eq, 3);
 ...
 * @param	S[3]      	  Output: sliding manifold values for the three surfaces used for the attitude control
 */
void SlidingMode::Step(const float q[4], const float dq[4], const float xy[2], const float dxy[2], const float COM[3], const float q_ref[4], const float omega_ref[3], const float velocity_reference[2], const bool VelocityReferenceInHeadingFrame, const float equivalentControlPct, float tau[3], float S[3])
{
	float dt;
	...
	Step(q, dq, xy, dxy, q_ref, omega_ref, velocity_reference, VelocityReferenceInHeadingFrame, _params.model.Jk, _params.model.Mk, _params.model.rk, _params.model.Mb, _params.model.Jbx, _params.model.Jby, _params.model.Jbz, _params.model.Jw, _params.model.rw, _params.model.Bvk, _params.model.Bvm, _params.model.Bvb, _params.model.l, _params.model.g, COM[0], COM[1], COM[2], _params.controller.K, _params.controller.Kv, _params.controller.Kvi, _params.controller.eta, _params.controller.epsilon, _params.controller.gamma, _params.controller.ContinousSwitching, _params.controller.EquivalentControl, equivalentControlPct, _params.controller.DisableQdotInEquivalentControl,  _params.controller.DisableOmegaXYInEquivalentControl, dt, tau, S);
}
 ...
 * @param	S[3]      	  Output: sliding manifold values for the three surfaces used for the attitude control
 */
void SlidingMode::Step(const float q[4], const float dq_in[4], const float xy[2], const float dxy[2], const float q_ref_in[4], const float omega_ref_body[3], const float velocity_reference[2], const bool VelocityReferenceInHeadingFrame, const float Jk, const float Mk, const float rk, const float Mb, const float Jbx, const float Jby, const float Jbz, const float Jw, const float rw, const float Bvk, const float Bvm, const float Bvb, const float l, const float g_const, const float COM_X, const float COM_Y, const float COM_Z, const float Kq[3], const float Kv[2], const float Kvi[2], const float eta[3], const float epsilon[3], const float gamma, const bool continuousSwitching, const bool IncludeEquivalentControl, const float equivalentControlPct, const bool DisableQdotInEquivalentControl, const bool DisableOmegaXYInEquivalentControl, const float dt, float tau[3], float S[3])
{
    // See ARM-CMSIS DSP library for matrix operations: https://www.keil.com/pack/doc/CMSIS/DSP/html/group__groupMatrix.html
    arm_matrix_instance_f32 q_; arm_mat_init_f32(&q_, 4, 1, (float32_t *)q);
    float dq[4] = {dq_in[0], dq_in[1], dq_in[2], dq_in[3]}; arm_matrix_instance_f32 dq_; arm_mat_init_f32(&dq_, 4, 1, (float32_t *)dq);
    ...
    /* Extract quaternion part of non-linear ODE */
    arm_matrix_instance_f32 fq_; arm_mat_init_f32(&fq_, 4, 1, &f[2]);
    float gq[4*3]; arm_matrix_instance_f32 gq_; arm_mat_init_f32(&gq_, 4, 3, gq);
    Matrix_Extract(g, 6,3,  2,0,  4,3,  gq);

    /* Extract position part of non-linear ODE */
    arm_matrix_instance_f32 fxy_; arm_mat_init_f32(&fxy_, 2, 1, &f[0]);
    float gxy[2*3]; arm_matrix_instance_f32 gxy_; arm_mat_init_f32(&gxy_, 2, 3, gxy);
    Matrix_Extract(g, 6,3,  0,0,  2,3,  gxy);
	...
	/* Compute velocity reference in heading frame */
	float velocity_reference_heading[2];
	if (VelocityReferenceInHeadingFrame) {
		velocity_reference_heading[0] = velocity_reference[0];
		velocity_reference_heading[1] = velocity_reference[1];
	} else {
		float heading = HeadingFromQuaternion(q);
		Math_Rotate2D(velocity_reference, -heading, velocity_reference_heading); // rotate velocity reference from inertial frame to heading frame
	}
	...
	/* Construct inverse input mapping matrix, W  (based on kinematics, hence wheel and ball radius and motor placements) */
	const float invW[3*3] = {
			  -0.365429861078319,   0.182714930539160,   0.182714930539160,
			   0.000000000000000,  -0.316471542995243,   0.316471542995243,
			   0.182714930539160,   0.182714930539160,   0.182714930539160
	};

	/* Compute InputInv matrix that maps from switching law to control output */
	float tmp3[3]; arm_matrix_instance_f32 tmp3_; arm_mat_init_f32(&tmp3_, 3, 1, tmp3);
	float tmp2x2[2*2]; arm_matrix_instance_f32 tmp2x2_; arm_mat_init_f32(&tmp2x2_, 2, 2, tmp2x2);
	float tmp2x3[2*3]; arm_matrix_instance_f32 tmp2x3_; arm_mat_init_f32(&tmp2x3_, 2, 3, tmp2x3);
	float tmp3x2[3*2]; arm_matrix_instance_f32 tmp3x2_; arm_mat_init_f32(&tmp3x2_, 3, 2, tmp3x2);
	float tmp3x3[3*3]; arm_matrix_instance_f32 tmp3x3_; arm_mat_init_f32(&tmp3x3_, 3, 3, tmp3x3);
	float tmp4x4[4*4]; arm_matrix_instance_f32 tmp4x4_; arm_mat_init_f32(&tmp4x4_, 4, 4, tmp4x4);
	float tmpPhi[4*4]; arm_matrix_instance_f32 tmpPhi_; arm_mat_init_f32(&tmpPhi_, 4, 4, tmpPhi);
	float tmpGamma[4*4]; arm_matrix_instance_f32 tmpGamma_; arm_mat_init_f32(&tmpGamma_, 4, 4, tmpGamma);
	float EKv[3*2] = { 0,    -Kv[0],
					  Kv[1],   0,
	                   0,      0};
	arm_matrix_instance_f32 EKv_; arm_mat_init_f32(&EKv_, 3, 2, EKv);
	float Input[3*3]; arm_matrix_instance_f32 Input_; arm_mat_init_f32(&Input_, 3, 3, Input);
	float InputInv[3*3]; arm_matrix_instance_f32 InputInv_; arm_mat_init_f32(&InputInv_, 3, 3, InputInv);

	/* InputInv = inv(gamma*invW + devec*Phi(q_ref)'*gq + E*Kv*devec2d*Phi(q)'*Gamma(q)*vec2d*gxy); */
	Quaternion_mat_PhiT(q, tmpPhi);
	Quaternion_mat_Gamma(q, tmpGamma);
	arm_mat_mult_f32(&tmpPhi_, &tmpGamma_, &tmp4x4_); // Phi(q)'*Gamma(q)
	Matrix_Extract(tmp4x4, 4,4,  1,1,  2,2,  tmp2x2); // devec2d*Phi(q)'*Gamma(q)*vec2d
	arm_mat_mult_f32(&tmp2x2_, &gxy_, &tmp2x3_); // devec2d*Phi(q)'*Gamma(q)*vec2d*gxy
	arm_mat_mult_f32(&EKv_, &tmp2x3_, &Input_); // Input = E*Kv*devec2d*Phi(q)'*Gamma(q)*vec2d*gxy

    arm_mat_mult_f32(&devecPhiQref_T_, &gq_, &tmp3x3_); //  devec*Phi(q_ref)'*gq
    arm_add_f32(tmp3x3, Input, Input, 3*3); // Input = devec*Phi(q_ref)'*gq + E*Kv*devec2d*Phi(q)'*Gamma(q)*vec2d*gxy

    arm_scale_f32((float*)invW, -gamma, tmp3x3, 3*3);
    arm_add_f32(tmp3x3, Input, Input, 3*3); // Input = gamma*invW + devec*Phi(q_ref)'*gq + E*Kv*devec2d*Phi(q)'*Gamma(q)*vec2d*gxy

    inv3x3(Input, InputInv);

    /* Compute velocity sliding variable, Sv */
    /* Sv = E*Kv*devec2d*Phi(q)'*Gamma(q)*vec2d*dxy - E*Kv*v_ref */
    float Sv[3]; arm_matrix_instance_f32 Sv_; arm_mat_init_f32(&Sv_, 3, 1, Sv);

    arm_matrix_instance_f32 velocity_reference_heading_; arm_mat_init_f32(&velocity_reference_heading_, 2, 1, (float*)velocity_reference_heading);
    arm_mat_mult_f32(&EKv_, &velocity_reference_heading_, &Sv_); // E*Kv*v_ref
    arm_negate_f32(Sv, Sv, 3); // Sv = -E*Kv*v_ref

    // This part basically rotates the current velocity into heading frame
    arm_matrix_instance_f32 dxy_; arm_mat_init_f32(&dxy_, 2, 1, (float*)dxy);
	Quaternion_mat_PhiT(q, tmpPhi);
	Quaternion_mat_Gamma(q, tmpGamma);
	arm_mat_mult_f32(&tmpPhi_, &tmpGamma_, &tmp4x4_); // Phi(q)'*Gamma(q)
	Matrix_Extract(tmp4x4, 4,4,  1,1,  2,2,  tmp2x2); // devec2d*Phi(q)'*Gamma(q)*vec2d
	arm_mat_mult_f32(&EKv_, &tmp2x2_, &tmp3x2_); // E*Kv*devec2d*Phi(q)'*Gamma(q)*vec2d
	arm_mat_mult_f32(&tmp3x2_, &dxy_, &tmp3_); // E*Kv*devec2d*Phi(q)'*Gamma(q)*vec2d*dxy

	arm_add_f32(tmp3, Sv, Sv, 3); // Sv = E*Kv*devec2d*Phi(q)'*Gamma(q)*vec2d*dxy - E*Kv*v_ref

    /* Compute sliding variable, S */
		/* S = devec*Phi(q_ref)'*dq
			 + Kq*devec*Phi(q_ref)'*q
  	  	 + Sv
  	  	 + Kvi * Sv_integral        from "Springer - Sliding mode.pdf"
         + gamma*tau_prev		    from "Discrete Integral Sliding Mode for Non-minimum phase systems.pdf"
	*/
	arm_matrix_instance_f32 S_; arm_mat_init_f32(&S_, 3, 1, S);

	arm_mat_mult_f32(&devecPhiQref_T_, &dq_, &S_); // S = devec*Phi(q_ref)'*dq

	arm_mat_mult_f32(&devecPhiQref_T_, &q_, &tmp3_); // devec*Phi(q_ref)'*q
	arm_mult_f32(tmp3, (float*)Kq, tmp3, 3); // Kq*devec*Phi(q_ref)'*q
	arm_add_f32(tmp3, S, S, 3); // S += Kq*devec*Phi(q_ref)'*q

	arm_add_f32(Sv, S, S, 3); // S += Sv

	// S += Kvi * Sv_integral
	if (Kvi[0] != 0)
		S[0] += Kvi[0] * Sv_integral[0];
	else
		Sv_integral[0] = 0;

	if (Kvi[1] != 0)
		S[1] += Kvi[1] * Sv_integral[1];
	else
		Sv_integral[1] = 0;

	// S += gamma*invW*tau_prev
	arm_matrix_instance_f32 tau_prev_; arm_mat_init_f32(&tau_prev_, 3, 1, tau_prev);
	arm_scale_f32((float*)invW, -gamma, tmp3x3, 3*3);
	arm_mat_mult_f32(&tmp3x3_, &tau_prev_, &tmp3_);
	S[0] += tmp3[0];
	S[1] += tmp3[1];
	S[2] += tmp3[2];

	/* Update integral */
	Sv_integral[0] += dt * fmax(fmin(Sv[0], 1), -1);
	Sv_integral[1] += dt * fmax(fmin(Sv[1], 1), -1);

    /* Compute equivalent control */
    /* tau_eq = InputInv * (
							- devec*Phi(q_ref)'*fq
							- Kq*devec*Phi(q_ref)'*dq
							- E*Kv*devec2d*Phi(dq)'*Gamma(q)*vec2d*dxy
							- E*Kv*devec2d*Phi(q)'*Gamma(dq)*vec2d*dxy
							- E*Kv*devec2d*Phi(q)'*Gamma(q)*vec2d*fxy
							- Kvi * Sv
							)
	*/
    float sum3[3]; arm_matrix_instance_f32 sum3_; arm_mat_init_f32(&sum3_, 3, 1, sum3);
	...
	if (!DisableQdotInEquivalentControl) {
		arm_mat_mult_f32(&devecPhiQref_T_, &dq_, &tmp3_); // devec*Phi(q_ref)'*dq
		arm_mult_f32(tmp3, (float*)Kq, tmp3, 3); // Kq*devec*Phi(q_ref)'*dq
		arm_add_f32(tmp3, sum3, sum3, 3); // sum += Kq*devec*Phi(q_ref)'*dq

		Quaternion_mat_PhiT(dq, tmpPhi);
		Quaternion_mat_Gamma(q, tmpGamma);
		arm_mat_mult_f32(&tmpPhi_, &tmpGamma_, &tmp4x4_); // Phi(dq)'*Gamma(q)
		Matrix_Extract(tmp4x4, 4,4,  1,1,  2,2,  tmp2x2); // devec2d*Phi(dq)'*Gamma(q)*vec2d
		arm_mat_mult_f32(&EKv_, &tmp2x2_, &tmp3x2_); // E*Kv*devec2d*Phi(dq)'*Gamma(q)*vec2d
		arm_mat_mult_f32(&tmp3x2_, &dxy_, &tmp3_); // E*Kv*devec2d*Phi(dq)'*Gamma(q)*vec2d*dxy
		arm_add_f32(tmp3, sum3, sum3, 3); // sum += E*Kv*devec2d*Phi(dq)'*Gamma(q)*vec2d*dxy

		Quaternion_mat_PhiT(q, tmpPhi);
		Quaternion_mat_Gamma(dq, tmpGamma);
		arm_mat_mult_f32(&tmpPhi_, &tmpGamma_, &tmp4x4_); // Phi(q)'*Gamma(dq)
		Matrix_Extract(tmp4x4, 4,4,  1,1,  2,2,  tmp2x2); // devec2d*Phi(q)'*Gamma(dq)*vec2d
		arm_mat_mult_f32(&EKv_, &tmp2x2_, &tmp3x2_); // E*Kv*devec2d*Phi(q)'*Gamma(dq)*vec2d
		arm_mat_mult_f32(&tmp3x2_, &dxy_, &tmp3_); // E*Kv*devec2d*Phi(q)'*Gamma(dq)*vec2d*dxy
		arm_add_f32(tmp3, sum3, sum3, 3); // sum += E*Kv*devec2d*Phi(dq)'*Gamma(dq)*vec2d*dxy
	}
	...
	Matrix_Extract(tmp4x4, 4,4,  1,1,  2,2,  tmp2x2); // devec2d*Phi(q)'*Gamma(q)*vec2d
	arm_mat_mult_f32(&EKv_, &tmp2x2_, &tmp3x2_); // E*Kv*devec2d*Phi(q)'*Gamma(q)*vec2d
	arm_mat_mult_f32(&tmp3x2_, &fxy_, &tmp3_); // E*Kv*devec2d*Phi(q)'*Gamma(q)*vec2d*fxy
	arm_add_f32(tmp3, sum3, sum3, 3); // sum += E*Kv*devec2d*Phi(q)'*Gamma(q)*vec2d*fxy
